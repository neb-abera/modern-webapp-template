...
Everything runs in Docker, everything is tested, and every change goes
through a pull request gated on the verification suite.
...
* Write tests first, from the entry point a user hits (an HTTP request, a
  page interaction). A change in behavior needs a test that fails without it.
* A response is a record that lists its fields. Never return an entity. An
  ...
  ssn, password, token, secret). If clients need one, list it in
